...
A real-time web-server log streaming, indexing, visualization, and monitoring pipeline built with **Apache Kafka**, **Elasticsearch**, **Kibana**, **Prometheus**, and **Grafana**, orchestrated via **Docker Compose**. The system is designed to be modular, scalable, and fault-tolerant for large-scale log processing. :contentReference[oaicite:0]{index=0}

---

## What this project does

1. **Preprocess raw web logs** (`access.log`) into structured **JSON lines**. :contentReference[oaicite:1]{index=1}  
2. **Stream JSON logs in real-time** to a Kafka topic using a **producer**. :contentReference[oaicite:2]{index=2}  
3. **Consume messages** from Kafka and **index** them into Elasticsearch in real time. :contentReference[oaicite:3]{index=3}  
4. **Visualize** indexed logs using Kibana dashboards. :contentReference[oaicite:4]{index=4}  
5. **Monitor** producers/consumers and pipeline health with Prometheus + Grafana (latency, throughput, etc.). :contentReference[oaicite:5]{index=5}  
...
The project is intended for large-scale web log analysis (the report references processing ~3.3GB logs and common fields like timestamps, methods, IPs, status, URLs, user agents). :contentReference[oaicite:6]{index=6}

---

## Architecture (Dockerized services)

This project runs everything in containers to keep setup consistent and portable. :contentReference[oaicite:7]{index=7}
...
Fault tolerance is improved using a 3-broker Kafka cluster and replication requirements (e.g., minimum in-sync replicas). :contentReference[oaicite:8]{index=8}
